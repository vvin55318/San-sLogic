// 第一次進入，載入資料
$.get("api.php?do=signin_data",function(re){
$("#data").html(re)
...
// 刪除功能
  $(".del").click(function(re){
    if(confirm("確定要刪除嗎?")){
      ...
      $.post("api.php?do=signin_del", chkData ,function(re){
        location.reload();
        alert(re);
      })
    }
...
// 修改事件
  $(".mdy").click(function(event){
    ...
    $.post("api.php?do=signin_mdy",{id},function(re){  
...
// 儲存變更事件
      $(".save").click(function(result){
        $.get("api.php?do=signin_save",function(re){
          $("#data").html(re)
        })
      })
    })
...
$("#add").click(function(){
  ...
  $("#cvr").html(`
  <form action="api.php?do=signin_add" method="post" class="d-flex flex-column justify-content-center" enctype="multipart/form-data">
          <p class="mt-4">新增登入頁廣告</p>
