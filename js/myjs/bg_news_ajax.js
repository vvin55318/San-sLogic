// 第一次進入，載入資料1
$.get("api.php?do=news_data_1",function(re){
$("#data_1").html(re)
...
// 刪除功能1
  $(".del_1").click(function(re){
    if(confirm("確定要刪除嗎?")){
      ...
      $.post("api.php?do=news_del_1", chkData ,function(re){
        location.reload();
        alert(re);
      })
    }
...
// 修改事件1
  $(".mdy_1").click(function(event){
    ...
    $.post("api.php?do=news_mdy_1",{id},function(re){  
...
// 儲存變更事件1
      $(".save_1").click(function(result){
        $.get("api.php?do=news_save_1",function(re){
          $("#data_1").html(re)
        })
      })
    })
...
// 第一次進入，新增資料2
$.get("api.php?do=news_data_2",function(re){
  $("#data_2").html(re)
...
$(".del_2").click(function(re){
  if(confirm("確定要刪除嗎?")){
    ...
    $.post("api.php?do=news_del_2", chkData ,function(re){
      location.reload();
      alert(re);
    })
  console.log(id);
...
// 修改事件2
  $(".mdy_2").click(function(event){
    ...
    event.target.parentNode.parentNode.style.background="#ddd"
    $(".mdy_1,.del_1,.mdy_2,.del_2").attr("style","display:none")
    $.post("api.php?do=news_mdy_2",{id},function(re){  
      event.target.parentNode.parentNode.innerHTML=re
        $("#add_1").attr("style","display:none")
        $("#add_2").attr("style","display:none")
...
// 儲存變更事件2
      $(".save_2").click(function(){
        $.get("api.php?do=news_save_2",function(re){
          $("#data_1").html(re)
        })
      })
    })
...
$("#add_1").click(function(){
  ...
  $("#cvr").html(`
  <form action="api.php?do=news_add_1" method="post" class="d-flex flex-column justify-content-center" enctype="multipart/form-data">
          <p class="mt-4">新增活動資訊</p>
...
$("#add_2").click(function(){
  ...
  $("#cover").addClass("d-flex")
  $("#cvr").html(`
  <form action="api.php?do=news_add_2" method="post" class="d-flex flex-column justify-content-center" enctype="multipart/form-data">
          <p class="mt-4">新增產品資訊</p>
          <p>圖片:<input type="file" name="img" id="imgload"></p>
          <p><img id="putimg" width="40%"></p>
          <p>標題:<input type="text" name="title"></p>
          <p>內容:<input type="text" name="info"></p>
